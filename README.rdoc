...
* Using the API
  - Index/Listing from database
    > To return a JSON object of all appointments saved in the database, use the URL:
        http://localhost:3000/ ### To change once on heroku
    ...
    > Examples:
        ...
        6. http://localhost:3000/appointments?year=2013&month=11&day=1&hour=11&min=00
            + Gives all appointments starting on November 1, 2013 at 11:00 AM exactly
        7. http://localhost:3000/appointments?month=11
            + Gives all appointments in November of the current year
        8. http://localhost:3000/appointments?first_name=brenda&year=2013&month=11
            + Gives all appointments for anyone with a first name of brenda during
              November 2013
        9. http://localhost:3000/appointments?first_name=brenda&start_time=11/1/13 11:00
  ...
  - Choosing a CRUD Method:
    ...
    > There are four options:
        ...
        >> delete
    > Example:
        >> http://localhost:3000/?m=delete&first_name=brenda&last_name=hardy
  ...
  - Updating an appointment
    ...
    > ###Will require more work because there needs to be two sets of variables
 ...
 * Put minimum length for newly created appointments
 * Save everything lowercase, search should be lowercased
 * Change from UTC to EST everywhere.
